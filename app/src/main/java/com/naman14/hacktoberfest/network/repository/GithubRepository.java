...
public class GithubRepository {
    ...
    public Observable<List<Issue>> findIssues(String language, String[] tags) {
        return apiManager.getSearchApi().findIssues(Utils.getHacktoberfestIssuesQuery(language, tags))
                .flatMap(new Func1<SearchResponse, Observable<List<Issue>>>() {
                    @Override
                    public Observable<List<Issue>> call(SearchResponse response) {
                        if (response != null && response.getItems() != null) {
